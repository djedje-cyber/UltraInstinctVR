...
public class VRTest : MonoBehaviour
{
	...
	public virtual void Initialize(){
		...
		// Vérifier si le fichier existe, puis le vider
		if (File.Exists(filePath)) {
			using (StreamWriter writer = new StreamWriter(filePath, false)) {
			// Le fait de ne rien écrire dans le fichier va simplement le vider
			}
	...
	protected void FetchControls(){
		...
		// Chemin complet du fichier
		string filePath = folderPath + "/FoundObject.txt";

		// Ouvrir le fichier pour ajouter des lignes (créera le fichier s'il n'existe pas)
		using (StreamWriter writer = new StreamWriter(filePath, true)) {
			...
			foreach(GameObject go in gos){
				...
				if(r != null && !controls.ContainsKey(go)){
					...
					// Créer une chaîne contenant le nom et la position de l'objet
					string objectInfo = go.name +":"+ go.transform.position;
					
					// Afficher dans la console et écrire dans le fichier
					Debug.Log(objectInfo);
					writer.WriteLine(objectInfo);
