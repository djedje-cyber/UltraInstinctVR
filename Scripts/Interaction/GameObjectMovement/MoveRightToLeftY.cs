...
public class MoveRightToLeftY : MonoBehaviour
{
    ...
    public float moveDistance = 5f;  // Distance à parcourir sur l'axe Y
    ...
    private void StartGrab(UnityEngine.XR.Interaction.Toolkit.Interactables.XRGrabInteractable interactable)
    {
        ...
        if (selectInteractable != null)
        {
            ...
            // Démarrer le mouvement vertical (haut-bas)
    ...
    // Coroutine pour déplacer l'objet en haut et en bas sur l'axe Y
    private IEnumerator MoveObjectUpDown()
    {
        ...
        Vector3 downPosition = startPosition;  // Revenir à la position d'origine

        // Déplacement vers le haut
        yield return MoveToPosition(grabbedObject, upPosition);
        Debug.Log("⬆️ Déplacement terminé vers le haut");

        // Déplacement vers le bas
        yield return MoveToPosition(grabbedObject, downPosition);
        Debug.Log("⬇️ Déplacement terminé vers le bas");
