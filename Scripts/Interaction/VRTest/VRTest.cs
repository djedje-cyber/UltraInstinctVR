...
public class VRTest : MonoBehaviour
{
		...
		using (StreamWriter writer = new StreamWriter(filePath, true)) {
			...
			foreach(GameObject go in gos){
				...
				if(r != null && !controls.ContainsKey(go)){
					...
					// Met à jour le timer lorsqu'un nouvel objet est trouvé
					lastInteractionTime = Time.time;
				}
			}
		}
		...
		if(hitflag){
			...
			if(controls.ContainsKey(hitObj)){
				...
				if(info.getTriggered() == 0){
					triggered = hitObj;
					pointClick(hitObj);
					lastInteractionTime = Time.time; // Mise à jour du timer
				}
			}
		}
	...
	private IEnumerator CheckTimeout()
	{
		while (isRunning)
		{
			...
			{
				Debug.Log("Aucun nouvel objet trouvé depuis 2 minutes, arrêt du jeu.");
				StopGame();
